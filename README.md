...
> 🧬 **Project status: v0.1 "Cell Genesis" complete + Phase 10 (API & config).** Cells → Tissues → Organs → Cortex, with persistent vector memory and a forgetting curve, voice (Ear + Mouth), gated tools with human `[y/N]` approval, a Development Organ that drafts patches it cannot merge, advanced learning (strategies compete under UCB; successes become remembered skills, failures remembered weaknesses; Brier/ECE audit the brain's honesty) — and now a **REST API**: `pip install -e ".[api]"`, then `python scripts/run_api.py` serves `/v1/chat`, memory, feedback, gated tools, and health over HTTP from a [TOML config](incortex.toml.example), fail-closed by default. 450 tests, 100% coverage, zero *required* dependencies. Walkthroughs for every phase live in [docs/understanding/](docs/understanding/); API reference in [docs/api_reference.md](docs/api_reference.md); build history in [ROADMAP.md](ROADMAP.md).
